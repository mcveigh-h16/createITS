            ...
            if seq_record.id in SSUcomplete['accession'].tolist(): 
                ...
                if seq_record.id in SSUbothpartial['accession'].tolist():
                    if (start == 1) & (stop == length):
                        ...
                        seq_record.description = "small subunit ribosomal RNA, internal transcribed spacer 1"
                        start = "<" + str(start)
                        stop = ">" + str(stop)
                        print(seq_record.id, " SSU is incomplete on both ends but the RNA prediction does not extend to the end, gene fragment or intron present")
        if seq_record.id in Five_RNA_df['accession'].tolist(): 
...
#fix for 5.8s only                        
                        ...
                        #print ("test ",seq_record.id, start)
        ...
        if seq_record.id in LSU_RNA_df['accession'].tolist():  
            if seq_record.id in LSUcomplete['accession'].tolist():
                ...
                stop = ">" + str(stop)
                if seq_record.id not in Five_RNA_df['accession'].tolist():
##this segment needs more testing for multiple LSU predictions
                    if seq_record.id in LSUbothpartial['accession'].tolist():
                         if (start == 1) & (stop == length):
                             ...
                             stop = ">" + str(stop)
                             seq_record.description = "large subunit ribosomal RNA"
                             print(seq_record.id, " LSU is incomplete on both ends but the RNA prediction does not extend to the end, gene fragment or intron present")
